...
\begin{example}{(Incorrect causal graph when only using $L_{fit, obs}$)}
    ...
    The model is the following function: $\hat{x}_{t+1}=x_t+10\mathds 1_{a=+y}-10\mathds 1_{a=-y}$, $\hat{y}_{t+1}=y_t+10\mathds 1_{a=+x}-10\mathds 1_{a=-x}$.
...
Thus, we naturally arrive at the $l_1$-relaxation of our discrete problem [cite Bengio].

We define this loss as:
$$
L_{sparse}=\mathbb K(B)=\sum\limits_{x\in X_{in}}\|P_x\|_1
$$

{\bf Exploration}. In case if for some of the component, $P_{xx'}=0$, that feature is {\em never} selected and is never active.
