...
{\bf Optimization scheme}. Now, we have a number of losses that we would like to minimize together: $L_{rec}$, $L_{fit, feat}$, $L_{fit, obs}$, $L_{sparse}$. Ideally, we would like to find a point which jointly minimizes all of these losses. In the case of a deterministic environment, it is possible to achieve all of the losses being $0$, $L_{sparse}$ having the minimal possible discrete value.
