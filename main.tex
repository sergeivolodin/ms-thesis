...
\begin{example}{(Discrete-valued variables)}
    Suppose that $y\sim Uniform([n])$ -- a discrete uniform variable taking values in $\{1,...,n\}$. $\mathbb Ey=n/2$ and $\var y=(n^2-1)/12$. In case if $\rho(y,\hat{y})<\varepsilon<\frac{1}{2n}$, we compute $P(y\neq \hat{y})=P(|y-\hat{y}|>1/n)$
\end{example}
...
In contrast, $p_{\min}>0$ would not have significant effects on the loss, as adding a non-important feature cannot improve the loss significantly. Therefore, it is safe to set a threshold on the lower value of the probabilities. This would help exploration.
...
Separate networks, custom gradient for gumbel, gin configuration, modularity
