...
Contrastive \cite{VanDenOord2018,Kipf2020,Mohammadi2020}
...
\begin{example}
    ...
    Now, implementing the $z\mod n$ function for $z\in [kn]$ with a neural network of constant complexity is impossible [?????] (unless using a periodic activation functions \cite{Sitzmann2020}).
...
We also note that the Non-Linear Independent Component Analysis\cite{Dinh2015,Hyvarinen2019} would not yield the desired representations in cases where there is significant dependence between features.
\todo{Add on why ICA fails}

\todo{Add images of graphs and environments}
...
\begin{itemize}
    ...
    \item Adaptive sparsity\cite{Dieleman2021} with a multiplicative update for the $\lambda$ parameter given the reconstruction losses. \todo{Describe adaptive multiplicative sparsity}. We note that this approach is ideologically similar to the Lagrangian update, but does not consider the magnitude of the reconstruction loss compared to the threshold.
\end{itemize}
...
\todo{Describe optimization scheme details.}
...
\todo{Update from Mendeley}
\label{ch:related_work}
%%%%%%%%%%%%%%%%%%%%%%

GOFA approaches with model simplicity as a prior. \cite{Ave, DesJardins1993,Evans2018} considers a symbolic reasoning approach. \cite{Jonsson2006} considers grouping states into "meta states". \cite{Corneil2018} defines "prototype" states.

\cite{Thomas2018} consider a model where one action corresponds to a change in one feature.
...
\cite{Barnett2015} apply a {\em linear} decoder and learn a sparse linear model in the context of Granger causality.
A number of approaches learn the sparse graph, but without a learned representation (without a decoder network) \cite{Kalainathan2018,Fu2013,Ke2019,Chalupka2017,Javed2020,Battaglia2016}

Other approaches involve sparse feature vectors for interpretability \cite{Weinstein,Zhu2017,VandenOord2017,Cha2018,Deng,Liu2018,Javed2019,Rafati2019,Pei2019,Lillicrap2019,Petros2020,Fallah2020,Javed2020}

\cite{Madumal2019} generate natural language explanations given a causal model of the environment. Our work can be seen as complementary: we learn the causal graph that can be used as input for that method.
...
\cite{Nair2019} implicitly code for the edges of a causal graph in terms of a neural network, i.e. the network predicts which edges to add or remove. In contrast, we use a matrix of probabilities which determines the node adjacency matrix.


\cite{Yarats2016} cluster the environment's state space.

\cite{Johnson2016,Tang2019,Kurutach2021,Chalupka2015} learn a causal model jointly with a decoder, but do not evaluate the sparsity.

\cite{Ehsan2018} apply natural language explanations to Reinforcement Learning.

Simple mathematical equations can describe our world well \cite{Hamming1980}. We assume that the environments in RL that we are interested in are also describable by such equations.

\cite{Everitt2019} use the tools of causal reasoning to interpret agents.
...
\cite{Lino2021} sparsify recurrent networks in the context of compute-efficiency.

Libraries: pytorch-geometric, graph-nets from DeepMind

\cite{genewein2020algorithms} propose to use probability trees instead of graphical models to encode causal relationships. The benefit is that the set of parents of a node can be dynamically dependent on values of other nodes. The paper does not provide a way to construct the probability trees.

\cite{zambaldi2018relational} introduce an attention module inside a Reinforcement Learning agent (inside the policy and the value networks), allowing it to solve tasks not solved by a simple MLP, because the task requires relational reasoning. The attention on the charts seem to favor one object attended to, corresponding to the ground truth next item to go to. Similar technique can be seen in \cite{hahne2019attention} to solve visual reasoning tasks (similar to IQ tests)
...
\cite{Kalainathan2018, Ng2019} propose to learn a distribution over causal graph in a mean-field way (all edges are independent and modelled as Bernoulli distribution). \cite{brouillard2020differentiable} extend this approach to modelling interventions.

\cite{Fallah2020} use dictionary learning with an autoencoder.

\cite{Kipf2020} work on learning abstract representations from pixels in 3D environments and in Atari using Graph neural networks, leading to more interpretable world (environment) models.
