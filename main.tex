...
\begin{figure}
    ...
    \caption{The causal graph for VectorIncrement$(5)$ without a decoder or an encoder. First, each state component determines its own value at the next time-step. Secondly, a corresponding action increments the state component. Finally, all state components affect reward and done, and all but one action affect the reward. This one of 5 possible causal graphs for this environment (one of the actions can be disconnected from the reward node)}
    \label{fig:ve5nodec}
\end{figure}
